...
namespace UnitTests.Data
{
    ...
    public class TestShortScore
    {
        ...
        private readonly string testCategory = "category";
        ...
        {
            ShortScore score = new ShortScore(testSearchstring, testStarRating, testFetchTime, testCategory);
            Assert.IsTrue(score.StarRating.Stars == 5, "StarRating should be 5");
            Assert.IsTrue(score.Searchstring == testSearchstring.ToUpper(), "StarRating should be testSearchstring");
            Assert.IsTrue(score.FetchTime == testFetchTime, "FetchTime should be testFetchTime");
            Assert.IsTrue(score.FetchTime == testFetchTime, "FetchTime should be testFetchTime");
            Assert.IsTrue(score.Category == testCategory, "Category should be testCategory");
            score = new ShortScore(testSearchstring, null, testFetchTime, testCategory);
            Assert.IsNull(score.StarRating, "StarRating should be null");
            Assert.IsTrue(score.Pp == 0, "PP should be 0");
        }

        [TestMethod]
        public void QuintConstructor()
        {
            ShortScore score = new ShortScore(testSearchstring, testPp, testStarRating, testFetchTime, testCategory);
            Assert.IsTrue(score.StarRating.Stars == 5, "StarRating should be 5");
            Assert.IsTrue(score.Pp == testPp, "StarRating should be testPp");
            Assert.IsTrue(score.Searchstring == testSearchstring.ToUpper(), "StarRating should be testSearchstring");
            Assert.IsTrue(score.FetchTime == testFetchTime, "FetchTime should be testFetchTime");
            Assert.IsTrue(score.Category == testCategory, "Category should be testCategory");
            score = new ShortScore(testSearchstring, 0, null, testFetchTime, testCategory);
            Assert.IsNotNull(score.StarRating, "StarRating should be default");
            Assert.IsTrue(score.Pp == 0, "PP should be 0");
        }
    }
}
